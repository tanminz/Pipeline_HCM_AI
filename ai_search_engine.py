#!/usr/bin/env python3
"""
AI Search Engine - Advanced image search with CLIP, FAISS, and multiple AI models
Optimized for HCMC AI Challenge competition
"""
...
class AISearchEngine:
    def __init__(self, metadata_file="real_image_metadata.json"):
        ...
        # Index storage files
        self.index_file = "ai_engine_faiss_index.bin"
        self.features_file = "ai_engine_clip_features.npy"
        self.valid_ids_file = "ai_engine_valid_ids.pkl"
    ...
    def initialize_models(self):
        ...
        try:
            ...
            # Initialize sentence transformer for text queries
            ...
            logger.warning("Using fallback search methods")
    ...
    def load_or_build_index(self):
        ...
        try:
            ...
            if (os.path.exists(self.index_file) and 
                ...
                os.path.exists(self.valid_ids_file)):
            ...
            # Build new index if files don't exist
            logger.info("🔨 Building new FAISS index...")
            self.build_search_index()
            
        except Exception as e:
            logger.error(f"Error loading/building index: {e}")
            logger.info("Building new index due to error...")
            self.build_search_index()
    
    def save_index(self):
        """Save FAISS index and related files"""
        try:
            if self.faiss_index is not None and self.clip_features is not None:
                ...
                logger.info(f"  - Valid IDs file: {self.valid_ids_file}")
    ...
    def build_search_index(self):
        ...
        try:
            ...
            logger.info(f"✅ FAISS index built with {len(valid_images)} images")
    ...
    def search_by_image(self, image_path: str, k: int = 20) -> List[Dict]:
        """Search similar images using CLIP"""
        try:
            if self.faiss_index is None or self.clip_model is None:
                return []
            
            # Extract image features
            image_features = self.extract_clip_features(image_path)
            image_features = image_features.reshape(1, -1).astype(np.float32)
            
            # Normalize features
            faiss.normalize_L2(image_features)
            
            # Search
            similarities, indices = self.faiss_index.search(image_features, k)
    ...
    def advanced_search(self, query: str, filters: Dict = None, k: int = 50) -> List[Dict]:
        ...
        try:
            # Basic text search
            results = self.search_by_text(query, k)
    ...
    def apply_filters(self, results: List[Dict], filters: Dict) -> List[Dict]:
        ...
        for result in results:
            ...
            if 'source' in filters:
                source_zip = self.image_metadata[result['filename']].get('source_zip', '')
    ...
    def rerank_results(self, results: List[Dict], query: str) -> List[Dict]:
        ...
        try:
            # Boost results with better filename matches
    ...
    def get_search_statistics(self) -> Dict:
        ...
        return {
            'total_images': len(self.image_metadata),
            'indexed_images': len(self.valid_image_ids) if hasattr(self, 'valid_image_ids') else 0,
